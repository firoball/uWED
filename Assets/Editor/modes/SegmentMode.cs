...
namespace Editor.Modes
{
    public class SegmentMode : BaseEditorMode
    {
        ...
        private void FinishConstruction(Vertex final)
        {
            ...
            m_mapData.Remove(m_current, false, true); //Vertex was added another time by ConstructNewSegment
            m_current = null;
            m_newSegments.Clear();
        ...
        public override void ModifyObject(Vector2 mouseWorldPos, EditorView ev)
        {
            ...
            if (ci.HoverVertex != null &&
                ci.HoverVertex.Connections.Count == 2) //Join two segments (vertex must not have other connections)
            {
            ...
            if (m_mapData.FindSegment(n1, n2) == null) //joined segment must not exist already
            {
                ...
                if (valid)
                {
                    ...
                    foreach (Segment s in segments)
                        m_mapData.Remove(s);
                    m_mapData.Remove(v,
                        true); //if there's still a connection, it's a broken one - force vertex deletion
                }
            }
        ...
        {
            Vector2 vertexPos =
                Geom2D.ProjectPointToLine(mouseWorldPos, s.Vertex1.WorldPosition, s.Vertex2.WorldPosition);
