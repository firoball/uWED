    ...
    public abstract class BaseEditorDrawer : ImmediateModeElement
    {
        ...
        protected void DrawLine(Vector2 start, Vector2 end, Color color)
        {
            if (contentRect.Contains(start) || contentRect.Contains(end) ||
                contentRect.Contains(new Vector2(start.x, end.y)) ||
                contentRect.Contains(new Vector2(end.x, start.y))) //TODO: this can hide lines that are actually visible
            {
                GL.Begin(GL.LINES);
                GL.Color(color);
                GL.Vertex(start);
        ...
        {
            m_cursorInfo.Objects = m_mapData.Objects.Count;
            m_cursorInfo.Vertices = m_mapData.Vertices.Count;
            m_cursorInfo.Segments = m_mapData.Segments.Count;
            m_cursorInfo.Regions = m_mapData.Regions.Count;
            m_cursorInfo.Ways = m_mapData.Ways.Count;
